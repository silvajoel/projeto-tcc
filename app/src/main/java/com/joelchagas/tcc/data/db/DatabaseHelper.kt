...
class DatabaseHelper(private val context: Context) :
    ...
    fun insertGlicemia(
        ...
        val values = ContentValues().apply {
            ...
            put(DatabaseConstants.glicemia.GlicemiaColumns.OBSERVACOES, if (observacoes.isEmpty()) "Sem observações" else observacoes)
        }
    ...
    fun insertInsulina(
        ...
        val values = ContentValues().apply {
            ...
            put(DatabaseConstants.insulina.InsulinaColumns.OBSERVACOES, if (observacoes.isEmpty()) "Sem observações" else observacoes)
    ...
    fun getAllInsulinas(userId: Int): List<InsulinaData> {
        ...
        val query = "SELECT * FROM ${DatabaseConstants.insulina.TABLE_NAME} WHERE ${DatabaseConstants.insulina.InsulinaColumns.USER_ID} = ?"
        val cursor = db.rawQuery(query, arrayOf(userId.toString()))

        if (cursor != null) {
            while (cursor.moveToNext()) {
                val id = cursor.getInt(cursor.getColumnIndexOrThrow(DatabaseConstants.insulina.InsulinaColumns.ID))
                val userId = cursor.getInt(cursor.getColumnIndexOrThrow(DatabaseConstants.insulina.InsulinaColumns.USER_ID))
                val quantidade = cursor.getInt(cursor.getColumnIndexOrThrow(DatabaseConstants.insulina.InsulinaColumns.INSULINA_QUANTIDADE))
                val tipo = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseConstants.insulina.InsulinaColumns.TIPO_INSULINA))
                val dataHora = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseConstants.insulina.InsulinaColumns.DATA_HORA_APLICACAO))
                val observacoes = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseConstants.insulina.InsulinaColumns.OBSERVACOES))
                val momento = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseConstants.insulina.InsulinaColumns.MOMENTO_MEDICAO))

                val insulina = InsulinaData(id, userId, quantidade, tipo, dataHora, observacoes, momento)
    ...
    fun getAllGlicemias(userId: Int): List<GlicemiaData> {
        ...
        val db = readableDatabase
        val query = "SELECT * FROM ${DatabaseConstants.glicemia.TABLE_NAME} WHERE ${DatabaseConstants.glicemia.GlicemiaColumns.USER_ID} = ?"
        val cursor = db.rawQuery(query, arrayOf(userId.toString()))

        while (cursor.moveToNext()) {
            val id = cursor.getInt(cursor.getColumnIndexOrThrow(DatabaseConstants.glicemia.GlicemiaColumns.ID))
            val userIdFromDb = cursor.getInt(cursor.getColumnIndexOrThrow(DatabaseConstants.glicemia.GlicemiaColumns.USER_ID))
            val nivel_glicose = cursor.getInt(cursor.getColumnIndexOrThrow(DatabaseConstants.glicemia.GlicemiaColumns.NIVEL_GLICOSE))
            val data_hora_medicao = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseConstants.glicemia.GlicemiaColumns.DATA_HORA_MEDICAO))
            val observacoes = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseConstants.glicemia.GlicemiaColumns.OBSERVACOES))
            val momento_medicao = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseConstants.glicemia.GlicemiaColumns.MOMENTO_MEDICAO))
    ...
    fun getUsuarioNome(email: String): String {
        ...
        if (cursor.moveToFirst()) {
            nome = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseConstants.usuario.COLUMNS.NOME))
        }
        cursor.close()
        return nome
    }
    fun getUsuarioId(email: String, senha: String): Int? {
        val db = readableDatabase
        val selection = "${DatabaseConstants.usuario.COLUMNS.EMAIL} = ? AND ${DatabaseConstants.usuario.COLUMNS.SENHA} = ?"
        ...
        var userId: Int? = null
        if (cursor.moveToFirst()) {
            userId = cursor.getInt(cursor.getColumnIndexOrThrow(DatabaseConstants.usuario.COLUMNS.ID))
        }
        cursor.close()
        return userId
    }
    ...
    """
        ...
        if (cursor.moveToFirst()) {
            glicemiaAtual = cursor.getInt(cursor.getColumnIndexOrThrow(DatabaseConstants.glicemia.GlicemiaColumns.NIVEL_GLICOSE))
        }
        cursor.close()
